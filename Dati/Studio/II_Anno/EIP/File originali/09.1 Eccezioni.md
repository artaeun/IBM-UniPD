...
Errori sollevati quando l'interprete incontra una riga di codice che non può essere eseguita o una circostanza inattesa.

Quando un'eccezione viene sollevata, il programma non continua con l'enunciato successivo, ma con un gestore di eccezione (exception handler). Perciò nasce la necessità di gestire l'eccezione in maniera opportuna per evitare la terminazione del programma.
...
Fa sollevare un oggetto di tipo "RuntimeError" scritto dall'utente.
```python
if amount > balance:
